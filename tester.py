    ...
    def test(self):
        ...
        for i in range(batch_num):
            ...
            for k in range(self.batch_size):
                cv2.imwrite(os.path.join(self.test_label_path, str(i * self.batch_size + k) + '.png'),
                            labels_predict_plain[k])
                save_image(labels_predict_color[k],
                           os.path.join(self.test_color_label_path, str(i * self.batch_size + k) + '.png'))
